...
def main():
    ...
    pygame.display.set_caption("CartPole RL Agent - Modern Console")
    ...
    congrats_font = pygame.font.SysFont("Segoe UI", 48, bold=True)
    congrats_sub_font = pygame.font.SysFont("Segoe UI", 24, bold=True)
    ...
    # MODIFIED: Renamed draw_score to draw_header and added target display
    def draw_header(screen, score):
        """Draws the score and target in the header area with proper alignment."""
        ...
        # Target Score (Right-aligned in the left column)
        target_color = (0, 128, 0) if score <= 150 else (255, 215, 0) # Green, then Gold
        target_text = score_font.render("Target: >150", True, target_color)
        target_rect = target_text.get_rect(
            right=LEFT_COL_X + LEFT_COL_W, 
            centery=HEADER_Y + HEADER_H // 2
        )
        screen.blit(target_text, target_rect)

    def draw_congratulations(screen):
        """Draws the congratulations overlay."""
        ...
        win_text = congrats_sub_font.render("You Win! Score > 150!", True, (255, 255, 255))
        
        congrats_rect = congrats_text.get_rect(center=(WIN_W // 2, WIN_H // 2 - 40))
        win_rect = win_text.get_rect(center=(WIN_W // 2, WIN_H // 2 + 10))
    ...
    for episode_num in range(1, EPISODES + 1):
        ...
        for step in range(MAX_STEPS):
            ...
            draw_header(screen, ep_reward) # MODIFIED: Call the new header function
        ...
        if episode_won:
            print(f"YOU WIN! Episode {episode_num} reached score {ep_reward}!")
            ...
            print("Training stopped after achieving score > 150. Final model saved.")
            running = False
